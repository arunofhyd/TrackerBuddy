...
    // ADDED: Debounce helper function to limit the rate of function execution
...
            inputElement.setAttribute('aria-invalid', 'true'); // ADDED: Accessibility
        } else {
            inputElement.classList.remove('border-red-500', 'ring-red-500');
            inputElement.classList.add('border-gray-200');
            inputElement.removeAttribute('aria-invalid'); // ADDED: Accessibility
        }
    }
...
            }
            button.setAttribute('aria-busy', 'true'); // ADDED: Accessibility
        } else {
...
            button.style.width = '';
            button.style.height = '';
            button.removeAttribute('aria-busy'); // ADDED: Accessibility
        }
    }
...
        // ADDED: Accessibility role for announcements
        DOM.messageDisplay.setAttribute('role', 'alert');
        if (type === 'error') {
...
    // ADDED: Helper to update dynamic ARIA labels for navigation
    function updateNavButtonLabels() {
        const isMonthView = state.currentView === 'month';
        const prevBtn = document.getElementById('prev-btn');
        const nextBtn = document.getElementById('next-btn');
        if(prevBtn) prevBtn.setAttribute('aria-label', isMonthView ? 'Previous month' : 'Previous day');
        if(nextBtn) nextBtn.setAttribute('aria-label', isMonthView ? 'Next month' : 'Next day');
    }
...
        
        updateNavButtonLabels(); // ADDED: Update nav button labels on view change
...
            let leaveTypeName = ''; // MODIFIED: For ARIA label
            if(leaveData) {
...
                    leaveTypeName = `${leaveType.name} (${leaveData.dayType} day)`; // MODIFIED: For ARIA label
                }
            }
...
                    <div class="day-note-container">${noteText ? `<span class="day-note">${sanitizedNote}</span>` : ''}</div>
                    ${hasActivity ? '<div class="activity-indicator"></div>' : ''}
                </div>`;
            dayCell.dataset.date = dateKey;

            // MODIFIED: Add detailed ARIA label for accessibility
            let ariaLabel = formatDateForDisplay(dateKey);
            if (noteText) ariaLabel += `. Note: ${noteText}`;
            if (hasActivity) ariaLabel += '. This day has activities.';
            if (leaveTypeName) ariaLabel += `. Leave: ${leaveTypeName}.`;
...
            // This is the logic that generates default rows, and it is unchanged.
            for (let h = 8; h <= 17; h++) {
...
            // MODIFIED: Added specific ARIA labels to editable fields and buttons
            row.innerHTML = `
                <td class="py-3 px-4 whitespace-nowrap text-sm text-gray-900 cursor-text time-editable" data-time="${activity.time}" contenteditable="true" aria-label="Edit activity time: ${activity.time}">${activity.time}</td>
                <td class="py-3 px-4 text-sm text-gray-900">
                    <div class="activity-text-editable" data-time="${activity.time}" contenteditable="true" aria-label="Edit activity description">${formatTextForDisplay(activity.text)}</div>
                </td>
                <td class="py-3 px-4 text-sm flex space-x-1 justify-center items-center">
                    <button class="icon-btn move-up-btn" aria-label="Move activity up" ${isFirst ? 'disabled' : ''}>
...
                    </button>
                    <button class="icon-btn move-down-btn" aria-label="Move activity down" ${isLast ? 'disabled' : ''}>
...
            // ADDED: ARIA Label
            button.setAttribute('aria-label', `Select ${name} ${state.pickerYear}`);
            if (state.pickerYear === state.currentMonth.getFullYear() && index === state.currentMonth.getMonth()) {
                button.classList.add('bg-blue-500', 'text-white');
                button.classList.remove('bg-gray-100', 'text-gray-800');
                button.setAttribute('aria-current', 'true'); // ADDED: Accessibility
            }
...
                }
                successMessage = "Note saved."; // MODIFIED: Give feedback for debounced save
                break;
            }
            case 'ADD_SLOT': {
...
                        case 'LEAVE_TYPE':
                            if (detail1 && detail2 && detail3 && detail4) {
...
            <button type="button" data-color="${color}" style="background-color: ${color};" aria-label="Select color ${color}" class="w-10 h-10 rounded-full border-2 border-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"></button>
        `).join('');
    }

    function selectColorInPicker(color) {
        DOM.leaveColorPicker.querySelectorAll('button').forEach(btn => {
            // MODIFIED: Accessibility for selected state
...
        const selectedColorEl = DOM.leaveColorPicker.querySelector('[aria-pressed="true"]'); // MODIFIED: Use aria-pressed selector
...
            pill.setAttribute('aria-label', `Select leave type: ${lt.name}`); // ADDED: ARIA Label
            if (state.isLoggingLeave && state.selectedLeaveTypeId === lt.id) {
                pill.classList.add('ring-2', 'ring-offset-2', 'ring-blue-500');
                pill.setAttribute('aria-pressed', 'true'); // ADDED: Accessibility
            } else {
                pill.setAttribute('aria-pressed', 'false'); // ADDED: Accessibility
            }
...
            // MODIFIED: Added ARIA labels to icon buttons
            return `
                <div class="bg-white p-4 rounded-lg shadow relative border-2" style="border-color: ${lt.color};">
                    <div class="flex justify-between items-start">
                        <h4 class="font-bold text-lg truncate min-w-0 pr-2" style="color: ${lt.color};" title="${lt.name}">${lt.name}</h4>
                        
                        <div class="flex items-center -mt-2 -mr-2 flex-shrink-0">
                            <button class="move-leave-btn icon-btn" data-id="${lt.id}" data-direction="-1" aria-label="Move ${lt.name} leave type up" ${isFirst ? 'disabled' : ''}>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                            </button>
                            <button class="move-leave-btn icon-btn" data-id="${lt.id}" data-direction="1" aria-label="Move ${lt.name} leave type down" ${isLast ? 'disabled' : ''}>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>
                            <button class="edit-leave-type-btn icon-btn" data-id="${lt.id}" aria-label="Edit ${lt.name} leave type">
...
                pill.style.backgroundColor = lt.color;
                pill.textContent = lt.name;
                pill.setAttribute('aria-label', `Bulk apply leave type: ${lt.name}`); // ADDED: ARIA Label
...
    
            // MODIFIED: Added ARIA label to delete button
            item.innerHTML = `
...
                    </div>
                    <button class="delete-leave-day-btn text-red-500 hover:text-red-700 p-2 flex-shrink-0" aria-label="Remove leave for ${formatDateForDisplay(dateKey)}">
...
    // --- Event Delegation Setup for Daily View ---
...
        // MODIFIED: Debounced saving for the daily note
        const debouncedSaveNote = debounce((e) => {
            saveData({ type: 'SAVE_NOTE', payload: e.target.value });
        }, 500); // Wait 500ms after user stops typing
        DOM.dailyNoteInput.addEventListener('input', debouncedSaveNote);
        
...
            // ADDED: ARIA attribute update
            const isExpanded = !DOM.leaveStatsSection.classList.contains('hidden');
            DOM.statsToggleBtn.setAttribute('aria-expanded', isExpanded);
        });
        
...
                }
                DOM.infoToggleBtn.setAttribute('aria-expanded', !isExpanded); // ADDED: ARIA attribute update
            });
        }
    }
