...
def test_multi_select(page: Page):
    ...
    try:
        ...
        # Check for stronger bg color (bg-blue-200 is rgb(191, 219, 254))
        # Note: Playwright checks computed styles.
        # We can just check for class name if not running headless visual diffs.
        expect(page.locator("#multi-select-btn")).to_have_class(re.compile(r"bg-blue-200"))
        ...
        expect(page.locator("#weekend-option-modal")).to_contain_text("Weekends")
        ...
        # Verify range applied (first and last day should have dashed outline)
        # Note: applyRangeSelection sets isMultiSelectMode to false, but adds dates to leaveSelection set.
        # renderCalendar should show them as leave-selecting because isLoggingLeave is true.
        expect(days[0]).to_have_class(re.compile(r"leave-selecting"))
        expect(days[4]).to_have_class(re.compile(r"leave-selecting"))

        # Test Cancel Clearing
        # Click Cancel (Log Leave button which is now red and says Cancel)
        page.click("#log-new-leave-btn")

        # Verify dotted lines are gone
        expect(days[0]).not_to_have_class(re.compile(r"leave-selecting"))
        expect(days[4]).not_to_have_class(re.compile(r"leave-selecting"))

        # 5. Screenshot
        page.screenshot(path="verification/multi_select_final_check.png")

    except Exception as e:
        page.screenshot(path="verification/error_final_check.png")
        raise e
