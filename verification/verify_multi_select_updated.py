...
def test_multi_select(page: Page):
    ...
    try:
        ...
        # Toggle Sat to Exclude (Red)
        print("Clicking toggle-sat-btn")
        page.click("#toggle-sat-btn")

        # Verify attribute
        expect(page.locator("#toggle-sat-btn")).to_have_attribute("data-excluded", "true")
        ...
        # Debug: Check opacity
        opacity = page.locator("#weekend-option-modal").evaluate("el => getComputedStyle(el).opacity")
        print(f"Modal opacity: {opacity}")
        classes = page.locator("#weekend-option-modal").get_attribute("class")
        ...
        # 5. Screenshot
        page.screenshot(path="verification/multi_select_updated.png")
