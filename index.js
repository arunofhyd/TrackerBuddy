...
const { getSuperAdmins, assertAuthorized } = require("./utils");

admin.initializeApp();
const db = admin.firestore();

// Global options for v2 functions
const globalOpts = { region: "asia-south1", maxInstances: 10 };
...
exports.createTeam = onCall(globalOpts, async (request) => {
  if (!request.auth) {
...
exports.joinTeam = onCall(globalOpts, async (request) => {
  if (!request.auth) {
...
exports.editDisplayName = onCall(globalOpts, async (request) => {
  if (!request.auth) {
...
exports.editTeamName = onCall(globalOpts, async (request) => {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "You must be logged in.");
    }
...
exports.leaveTeam = onCall(globalOpts, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "You must be logged in.");
  }
...
exports.deleteTeam = onCall(globalOpts, async (request) => {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "You must be logged in.");
    }
...
exports.kickTeamMember = onCall(globalOpts, async (request) => {
...
exports.syncTeamMemberSummary = onCall(globalOpts, async (request) => {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "You must be logged in.");
    }
...
exports.updateMemberSummaryOnTeamChange = onDocumentWritten({ document: "teams/{teamId}", ...globalOpts }, async (event) => {
...
exports.getAllUsers = onCall(globalOpts, async (request) => {
    // Replaced repeated auth check with assertAuthorized
    await assertAuthorized(request, db);
...
exports.updateUserRole = onCall(globalOpts, async (request) => {
    const isSuperAdmin = await assertAuthorized(request, db);

    const { targetUserId, newRole, proExpiry } = request.data;
    // We need superAdmins for the permission check below
    const superAdmins = await getSuperAdmins(db);
...
exports.grantProByEmail = onCall(globalOpts, async (request) => {
    await assertAuthorized(request, db);
...
// Note: Auth triggers in v1 do not support runtime options in the same way as v2, using .runWith
...
exports.revokeProWhitelist = onCall(globalOpts, async (request) => {
    await assertAuthorized(request, db);
